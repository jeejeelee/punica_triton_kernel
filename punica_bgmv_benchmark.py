...
from kernels.bgmv_expand import bgmv_expand
from kernels.bgmv_shrink import bgmv_shrink
...
def test_triton_bgmv_punica_bgmv(
    ...
    #########################################################
    if op_type == "shrink":
        ################# triton_out_tensor bgmv_shrink bigen #############
        # warmup
        bgmv_shrink(
            inputs_tensor,
            lora_weights,
        ...
        record["triton_bgmv_shrink"].append(
            dict(
                dur_ms=dur_ms,
                batchs=batchs,
    ...
    #########################################################
        ...
        record["triton_bgmv_expand"].append(
            dict(
                dur_ms=dur_ms,
                batchs=batchs,
                hidden_size=hidden_size,
    ...
    # save
    with open(
        os.path.join(os.path.dirname(__file__), "record_bgmv_cudagraph.json"),
        "w",
        encoding="utf-8",
    ) as s:
...
def plot_results(bs=1, op_types="shrink"):
    ...
    triton_bgmv = results[f"triton_bgmv_{op_types}"]
    ...
    plt.plot(
        ...
        triton_bgmv,
        "o-",
        label=f"triton_bgmv_{op_types} bachsize={bs}",
        color="red",
    )
    plt.plot(
        hidden_size,
