...
HIDDEN_SIZES = [
    ...
    9216,
    10240,
    # When we use  sequence length of 512, furtherai_ops will raise an error (RTX3090),so comment
    # these dimension
    # 11008,
    # 13824,
    # 14336,
...
def test_triton_sgmv_punica_bgmv(
    ...
    if op_type == "shrink":
        ...
        record["further_sgmv_shrink"].append(
            dict(
                dur_ms=dur_ms,
                batchs=batchs,
                hidden_size=hidden_size,
    ...
    #########################################################
        ...
        record["further_sgmv_expand"].append(
            dict(
                dur_ms=dur_ms,
                batchs=batchs,
                hidden_size=hidden_size,
    ...
    # save
    with open(
        os.path.join(os.path.dirname(__file__), "record_sgmv_lee_cudagraph.json"),
        "w",
        encoding="utf-8",
    ) as s:
...
def plot_results(bs=1, op_types="shrink"):
    ...
    jee_bgmv = results[f"jee_sgmv_{op_types}"]
    ...
    plt.plot(
        ...
        jee_bgmv,
        "o-",
        label=f"jee_sgmv_{op_types} bachsize={bs}",
        color="red",
    )
    plt.plot(
        hidden_size,
